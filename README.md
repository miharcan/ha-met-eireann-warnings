...
## Concise dashboard card

This compact Markdown card shows land and marine warnings, hides environmental
advisories, and keeps each warning to a title plus one concise detail line.
It uses `card-mod` for spacing and visual grouping.
...
content: >
  ...
  <h1>Met Eireann Warnings</h1>

  {% if wanted | count == 0 %}
  <p>No active land or marine warnings.</p>
  {% else %}
  ...
  <section class="met-warning">
    <h3>{{ warning.display_title or warning.headline or warning.title }}</h3>
    <div class="met-meta">
      {{ warning.display_line }} -- {{ warning.display_summary or warning.description }}
    </div>
  </section>
  {% endfor %}
